...
├── database/            # Conexão (pool), schema.sql e seed.sql
...
3. Crie o banco de dados e aplique o schema:
   ...
   ```
4. (Opcional) Popule o banco com alguns dados de exemplo (autores, livros, clientes e empréstimos):
...
- [X] Cadastro, listagem, consulta, atualização e remoção de livros
- [X] Cadastro, listagem, consulta, atualização e remoção de clientes
- [X] Registro de empréstimos e devoluções, com controle de disponibilidade de exemplares
- [X] Relatórios (5 obrigatórios + 1 extra), usando JOIN, GROUP BY, ORDER BY, LIMIT e funções de agregação
...
Registrar um empréstimo (o prazo de devolução é calculado automaticamente, 14 dias a partir de hoje):
...
0. Voltar
Escolha uma opção: 1
1. A Hora da Estrela (Disponível: 1/2)
2. Capitães da Areia (Disponível: 1/2)
3. Cem Anos de Solidão (Disponível: 0/2)
4. Dom Casmurro (Disponível: 3/3)
5. Gabriela, Cravo e Canela (Disponível: 2/3)
Escolha um número da lista: 4
1. Ana Souza
2. Bruno Lima
3. Carla Ferreira
Escolha um número da lista: 1
Empréstimo registrado com sucesso:
#19 - "Dom Casmurro" para Ana Souza | Prazo: 31/07/2026
```

Consultar o relatório de livros disponíveis (usa `INNER JOIN` entre `livro` e `autor`):
...
0. Voltar
Escolha uma opção: 1
A Hora da Estrela - Clarice Lispector | Disponível: 1/2
Capitães da Areia - Jorge Amado | Disponível: 1/2
Dom Casmurro - Machado de Assis | Disponível: 2/3
Gabriela, Cravo e Canela - Jorge Amado | Disponível: 2/3
Harry Potter e a Câmara Secreta - J.K. Rowling | Disponível: 3/3
```
